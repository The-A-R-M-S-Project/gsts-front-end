...
const mutations = {
    ...
    reinviteExaminerSuccess(state, payload) {
        let str = payload.res
        let success = `${
            str[0].toUpperCase()
        }${
        ...
        state.studentsTableMessage = `${success}. You've reinvited ${payload.examiner} to assess ${payload.student}'s report`;
        state.displayInTable = false;
    },
...
const actions = {
    ...
    async reinviteExaminer({ commit }, data) {
        commit("setSubmitLoader", true)
        let accessToken = localStorage.getItem("jwt");
        axiosInstance.defaults.headers.common["Authorization"] = `Bearer ${accessToken}`;
        try {
            let response = await axiosInstance.post(`/report/staff/examiner/invite/${data.reportID}`, {examiner: data.examinerID});
            commit("reinviteExaminerSuccess", {
                res: response.data.status,
                examiner: data.examinerName,
                student: data.studentName
            });
